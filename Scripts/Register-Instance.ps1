...
# Determine the VSS volume from the data root path
$vssVolume = if ($dataRoot -match '^([A-Z]):') { "$($Matches[1]):\" } else { 'C:\' }
...
$vssVolumeToml = $vssVolume -replace '\\', '\\\\'
...
[paths]
torch_install   = "$torchPathToml"
world_save      = "$worldSavePathToml"
staging_local   = "$stagingPathToml"
data_root       = "$dataRootToml"

[smb]
share_name     = "$shareNameClean"
share_path     = "$stagingPathToml"
